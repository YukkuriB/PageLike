...
公开站点建议配置持久化的主缓存。基础点赞在 `CACHE_NONE` 下仍可使用，但标准 RateLimit 无法可靠工作。
...
`plrperiod` 可选 `7d`、`30d` 或 `all`，`plrlimit` 最大为 100。排行榜结果最多缓存 60 秒。
