...
- 安装 Echo 时，页面获得新点赞会通知页面创建者；同一用户对同一页面最多通知一次（不通知自赞或匿名创建者）
...
安装 Echo 后，页面创建者的站内通知默认开启，邮件通知默认关闭；用户可以在通知偏好中分别调整。只有真正新增的点赞会尝试触发通知，同一用户对同一页面最多通知一次，取消后重新点赞也不会重复通知。未安装 Echo 时会安全跳过通知，不影响其他功能。

为了防止反复取消和点赞刷屏，PageLike 会在第一次有效通知尝试时保存页面 ID 与点赞用户 ID 的内部去重标记。取消点赞会删除当前点赞状态，但不会删除该标记；标记不通过 API 或界面公开。页面删除、用户数据删除和孤儿清理会同步删除相关标记。
